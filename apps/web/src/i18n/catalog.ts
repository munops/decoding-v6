...
export const homeMessages: Record<Locale, HomeMessages> = {
  en: {
    ...
    intro: 'Paste unknown data. See likely formats, nested layers, evidence, and warnings.',
    uploaded: 'input bytes uploaded',
    examplesLabel: 'See the evidence with a known input',
  ...
  ko: {
    ...
    heading: '값의 정체와\n근거를 확인하세요.',
    intro:
      '값을 붙여넣으면 가능한 형식과 겹겹이 감싼 값, 각 판단의 근거와 경고를 함께 보여 줍니다.',
    uploaded: '서버로 전송된 입력 바이트',
    examplesLabel: '알려진 입력으로 판정 근거 보기',
  ...
  ja: {
    ...
    heading: '値をたどり、\n根拠を残す。',
    intro: '値を貼り付けると、候補形式、入れ子の層、各判定の根拠と警告を表示します。',
    uploaded: '送信バイト',
    examplesLabel: '既知の入力で判定根拠を見る',
  ...
  'zh-cn': {
    ...
    eyebrow: '无需账户的通用解码器',
    heading: '追溯这个值，\n保留判定依据。',
    intro: '粘贴未知值，即可查看候选格式、嵌套层以及每项判断的依据和警告。',
    uploaded: '上传字节',
    examplesLabel: '用已知输入查看判定依据',
  ...
  es: {
    ...
    intro:
      'Pega un valor desconocido para comparar formatos, seguir capas y revisar la evidencia de cada aviso.',
    uploaded: 'bytes subidos',
    examplesLabel: 'Mira la evidencia con una entrada conocida',
  ...
  'pt-br': {
    ...
    intro:
      'Cole um valor desconhecido para comparar formatos, seguir camadas e revisar a evidência de cada alerta.',
    uploaded: 'bytes enviados',
    examplesLabel: 'Veja as evidências com uma entrada conhecida',
  ...
  de: {
    ...
    intro:
      'Unbekannten Wert einfügen, Formate vergleichen, Ebenen verfolgen und die Belege jeder Warnung prüfen.',
    uploaded: 'hochgeladene Bytes',
    examplesLabel: 'Nachweise mit einer bekannten Eingabe ansehen',
  ...
  fr: {
    ...
    intro:
      'Collez une valeur inconnue pour comparer les formats, suivre les couches et examiner la preuve de chaque alerte.',
    uploaded: 'octets transférés',
    examplesLabel: 'Voir les preuves avec une entrée connue',
